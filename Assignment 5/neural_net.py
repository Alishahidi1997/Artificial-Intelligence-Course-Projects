...
class Neuron(DifferentiableElement):
    ...
    def compute_doutdx(self, elem):
        ...
        """
        dout = 0
        sigmoid_deriviation = self.output() * (1 - self.output())
        ...
        return dout * sigmoid_deriviation
...
class Regularization(PerformanceElem):
    def __init__(self, input, desired_value):
        assert isinstance(input, (Input, Neuron))
        DifferentiableElement.__init__(self)
        self.my_input = input
    ...
    def output(self):
        ...
        out = (-1/2) * ((self.my_desired_val - self.my_input.output()) ** 2) + _lambda * np.linalg.norm(np.asarray(_weights, dtype=np.float32))
        return out

    def dOutdX(self, elem):
        _lambda = 0.0001
        dout = (self.my_desired_val - self.my_input.output()) * self.my_input.dOutdX(elem) - 2 * _lambda * elem.get_value()
        return dout
...
# def make_neural_net_two_moonsss():
#     """
...
#     weights_out = []
#     all_weights = []
#     weights_out.append(Weight('wB', random_weight()))
#     for i in range(hidden_layer):
#         weights = []
#         if i < 10:
#             weights.append(Weight('w' + "A0" + str(i), random_weight()))
#         else:
#             weights.append(Weight('w' + "A" + str(i), random_weight()))
#         for j in range(1, 3):
#             if i < 10:
#                 weights.append(Weight('w' + str(j) + "A0" + str(i), random_weight()))
#             else:
#                 weights.append(Weight('w' + str(j) + "A" + str(i), random_weight()))
#         if i < 10:
#             neuron.append(Neuron("A0" + str(i), [i0, i1, i2], weights))
#             weights_out.append(Weight('w' + 'A0' + str(i) + 'B', random_weight()))
#         else:
#             neuron.append(Neuron("A" + str(i), [i0, i1, i2], weights))
#             weights_out.append(Weight('w' + 'A' + str(i) + 'B', random_weight()))
#         for i in weights:
#             all_weights.append(i)
#         all_weights.append(weights_out[len(weights_out) - 1])
#     print(neuron)
#     input_output = []
#     input_output.append(i0)
#     for i in neuron:
#         input_output.append(i)
#     B = Neuron('B', input_output, weights_out)
#     neuron.append(B)
#     # neuron.pop(0)
#     # print(len(all_weights))
#     all_weights = []
#     # P = Regularization(B, 0.0)
#     # P.set_weights(all_weights)
#
#     P = PerformanceElem(B, 0.0)
#     # for i in neuron:
#     #     print(i.get_weight_nodes())
#     net = Network(P, neuron)
#     return net
...
def make_neural_net_two_moons():
    ...
    for i in range(hidden_layer):
        if i < 10:
            weights["A0" + str(i)] = [Weight('w' + "A0" + str(i), random_weight())]
            for j in range(1, 3):
                weights["A0" + str(i)].append(Weight('w' + str(j) + "A0" + str(i), random_weight()))
        else:
            weights["A" + str(i)] = [Weight('w' + "A" + str(i), random_weight())]
            for j in range(1, 3):
                weights["A" + str(i)].append(Weight('w' + str(j) + "A" + str(i), random_weight()))

    for i in range(hidden_layer):
        if i < 10:
            neurons.append(Neuron("A0" + str(i), [i0, i1, i2], weights["A0" + str(i)]))
            out_weights.append(Weight('w' + 'A0' + str(i) + 'B', random_weight()))
        else:
            neurons.append(Neuron("A" + str(i), [i0, i1, i2], weights["A" + str(i)]))
            out_weights.append(Weight('w' + 'A' + str(i) + 'B', random_weight()))
    print(neurons)
    out_weights.insert(0, Weight('wB', random_weight()))
    input_output = []
    input_output.append(i0)
    for i in neurons:
        input_output.append(i)
    neurons.append(Neuron('B', input_output, out_weights))
    P = PerformanceElem(neurons[len(neurons) - 1], 0.0)


    all_weights = []
    all_weights.extend(out_weights)
    for item in list(weights.values()):
        all_weights.extend(item)
    P = Regularization(neurons[len(neurons) - 1], 0.0)
    P.set_weights(all_weights)
    net = Network(P, neurons)

    return net

def plot_decision_boundar(network, data, xmin = -2, xmax= 2, ymin= -2, ymax= 2):
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    steps = 300
    xmin = np.min(data) - 0.5
    ymin = np.min(data) - 0.5
    xmax = np.max(data) + 0.5
    ymax = np.max(data) + 0.5
    ...
    for datum in data:
        if datum[-1] < 0.5:
            ax1.scatter(datum[0], datum[1], s=10, c='k', marker="o", label=' < 0.5')
        else:
            ax1.scatter(datum[0], datum[1], s=10, c='r', marker="o", label=' >= 0.5')
    plt.show()
...
def train(network,
    ...
    # plot_decision_boundar(network, data)
        ...
        if prediction == datum[-1]:
            ...
            if verbose:
                print("test(%s) returned: %s => %s [%s]" %(str(datum),
                                                           ...
                                                           datum[-1],
                                                           "wrong"))
    print("Accuracy: ", float(correct)/len(data))
    plot_decision_boundar(network, data)
